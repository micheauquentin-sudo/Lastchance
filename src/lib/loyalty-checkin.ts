 ...
 * HMAC-SHA256, comparaison à temps constant, rotation de secret supportée
 * (LOYALTY_CHECKIN_TOKEN_SECRET, repli SPIN_TOKEN_SECRET).
 */
...
export function signLoyaltyCheckin(
  ...
  return { token: `${body}.${hmac(body, secret)}`, expiresAt };
}
