...
export const getUserAndOrg = cache(async () => {
  ...
  const { data: rows } = await supabase
    ...
    .select("organization_id, role, created_at, organizations(id, name, slug, stripe_customer_id, subscription_status, plan, trial_ends_at, past_due_since, logo_url, auto_reengage, notify_on_win, data_retention_months, webhook_url, timezone, addon_pronostics, addon_hunts, addon_loyalty, addon_jackpot, comp_access, comp_access_until, created_at)")
    .eq("user_id", user.id)
