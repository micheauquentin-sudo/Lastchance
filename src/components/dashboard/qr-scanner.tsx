...
/**
 * Scanner caméra de QR réutilisable : bascule caméra, décode chaque frame
 * (BarcodeDetector natif ou repli jsQR) et remonte la valeur brute via
 * `onResult`. Repli silencieux vers la saisie manuelle si la caméra est
 * refusée/indisponible (le composant se masque). Fonctionne partout où
 * getUserMedia existe.
 *
 * Aucune interprétation du contenu ici : l'appelant décide quoi faire de la
 * valeur (router vers la caisse, valider une visite fidélité, etc.).
 */
export function QrScanner({
  label,
  onResult,
  videoLabel = "Aperçu caméra pour scanner un QR code",
}: {
  label: string;
  onResult: (rawValue: string) => void;
  /** Libellé accessible de la vidéo (contexte du scan). */
  videoLabel?: string;
}) {
