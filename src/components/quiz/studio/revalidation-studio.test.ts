...
/**
 * CHAQUE REVALIDATION DE L'ATELIER A SON JUMEAU DE STUDIO (VIT-41).
 *
 * ── LE DÉFAUT QUE CETTE GARDE FERME, ET IL A DÉJÀ ÉTÉ LIVRÉ DEUX FOIS ──
 *
 * Le studio du quiz vit à `/studio/quiz/[id]`, HORS de `/dashboard`. Aucun
 * `revalidatePath("/dashboard/quiz/…")` ne l'atteint : Next revalide un CHEMIN,
 * pas une ressource. Une action qui réussit laisse donc l'écran afficher la
 * version d'avant — et sur un studio, où l'on enregistre en regardant, c'est
 * exactement l'endroit où le commerçant vient vérifier.
 *
 * Ce n'est pas une hypothèse : c'est le défaut VIT-37, puis VIT-39, mot pour
 * mot. `revalidatePath("/dashboard", "layout")` n'atteignait pas
...
const ACTION = join(__dirname, "..", "..", "..", "actions", "quiz.ts");
...
describe("le studio du quiz est revalidé partout où l'atelier l'est", () => {
  ...
  it("l'action revalide bien des chemins d'atelier — sinon la garde est vacante", () => {
    ...
    const atelier = chemins.filter((c) => c.startsWith("/dashboard/quiz/"));
    expect(atelier.length).toBeGreaterThanOrEqual(10);
  });

  it("chaque `/dashboard/quiz/${…}` a son `/studio/quiz/${…}`", () => {
    const manquants: string[] = [];

    for (const chemin of chemins) {
      const suffixe = chemin.replace(/^\/dashboard\/quiz\//, "");
      if (suffixe === chemin) continue; // pas un chemin d'atelier détaillé
      const jumeau = `/studio/quiz/${suffixe}`;
