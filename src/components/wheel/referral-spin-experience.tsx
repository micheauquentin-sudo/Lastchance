...
import { consumeReferralSpin, type ReferralSpinOutcome } from "@/actions/referral";
...
/**
 * Tour de roue offert par un versement `spin` de parrainage (récompense parrain,
 * bonus filleul ou coffre). Miroir EXACT de CalendarSpinExperience — même
 * animation (WheelSvg / WheelPointer), même flux de gain (ClaimForm → claimPrize,
 * code GAIN-…) : le résultat est décidé côté serveur par `consumeReferralSpin`,
 * on ne fait qu'animer vers le segment renvoyé. Seule différence : la roue publique
 * peut être en thème « nuit », alors le tour offert s'affiche dans un calque plein
 * écran AUTONOME (fond kermesse), pour rester lisible quel que soit le thème.
 */
 ...
 * consume_referral_spin_grant sort sans consommer le grant, le tour reste offert.
 */
type Phase = "ready" | "spinning" | "won" | "lost" | "kept" | "error";

export function ReferralSpinExperience({
  slug,
  grantToken,
  segments,
  claimConfig,
  ...
  /** Slug de la campagne roue (résout la campagne côté action). */
  slug: string;
  grantToken: string;
  ...
  const [outcome, setOutcome] = useState<ReferralSpinOutcome | null>(null);
  ...
  async function launch() {
    ...
    const result = await consumeReferralSpin({ slug, grantToken });
    if (!result.ok) {
    ...
    // Rien de tirable sur la roue (lots à stock illimité exclus du tour offert,
    // ou stocks vidés) : le grant N'A PAS été consommé.
  ...
  return (
    <div className="fixed inset-0 z-50 overflow-y-auto overscroll-contain bg-k-bg">
      <section className="mx-auto max-w-md px-4 py-8 text-center">
        ...
        <h1 className="mt-1 text-2xl font-black leading-tight text-k-ink">
          {rewardLabel || "Ton tour de roue offert"}
        </h1>
          ...
          <div role="status" aria-live="polite" className="mt-6">
            ...
            {outcome.claimToken ? (
              ...
              <p className="text-sm text-k-body">
                Présente cet écran au comptoir pour récupérer ton gain.
              </p>
            )}
        ...
        {phase === "lost" && (
          <div role="status" aria-live="polite" className="mt-6">
            ...
            <p className="text-k-body">
              La roue ne t&apos;a rien donné cette fois. Merci d&apos;avoir joué —
              et continue à faire grandir ton équipe !
            </p>
            <BackButton onExit={onExit} />
          </div>
        )}
        ...
        {phase === "kept" && (
          <div role="status" aria-live="polite" className="mt-6">
            ...
            <p className="text-k-body">
              ...
              ton tour reste offert : réessaie plus tard.
            </p>
            <BackButton onExit={onExit} />
          </div>
        )}
        ...
        {phase === "error" && (
          <div role="alert" className="mt-6">
            ...
            {/* Aucun refus de consumeReferralSpin ne consomme le grant : le tour
                reste acquis. */}
            ...
            <p className="mt-2 text-sm text-k-body">
              Ton tour reste offert — réessaie.
            </p>
