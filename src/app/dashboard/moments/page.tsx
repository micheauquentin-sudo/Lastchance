...
/**
 * LES MOMENTS — ateliers, dégustations, files d'accueil, invitations, offres de
 * dernière minute.
 *
 * ── POURQUOI CE CORPS DE PAGE N'EST PLUS PARTAGÉ (RDV-13) ──
 *
 * Il l'a été : une seule `PageAgenda({ mode })` servait « Réservation » et
 * « Moments », parce que les deux produits partagent les mêmes tables et que
 * seul `reservation_activities.booking_mode` les sépare. Le partage tenait tant
 * que les deux écrans posaient la MÊME question — « laquelle de vos
 * activités ? ». Il a cessé de tenir le jour où Réservation a cessé d'être une
 * liste : un restaurateur n'a pas un catalogue de salles, il en a UNE, et son
 * écran doit l'être. Un axe à deux valeurs qui ne décrit plus qu'un seul écran
 * n'abstrait plus rien : c'est un paramètre mort, et il coûte à chaque lecture.
 *
 * Ce qui reste commun aux deux écrans — l'enregistrement des arrivées — est un
 * COMPOSANT, pas une page : `ArriveesCheckin` se monte des deux côtés sans rien
 * à tenir d'accord.
 *
 ...
 * archivées. Un Moment est un objet de catalogue — « Dégustation », « Atelier
 * floral » — dont on tient trois ou quatre, et qui ne s'archive pas : il se
 * coupe. Un filtre par statut sur deux valeurs et une pagination sur quatre
 * lignes ajouteraient deux contrôles à lire pour rien. Le jour où un commerçant
 * en tient trente, ces deux composants existent et s'ajoutent sans rien changer
 * d'autre.
 */
export default async function MomentsPage() {
  const { organization } = await getUserAndOrg();

  // Découvrir / préparer / publier (cahier §3). Le droit s'appelle `reserver` —
  // la clé n'a pas changé de sens le jour où son libellé est devenu « Moments »
  // (migration 20261020120000). La prise de rendez-vous, elle, a le sien.
  ...
  // d'accueil, les offres de stock — et n'ont aucune donnée en commun : les
  // enchaîner aurait ajouté deux allers-retours à une page que le commerçant
  // ouvre en début de service.
  const [agenda, filesAccueil, offresStock] = await Promise.all([
    loadReserverDashboardContext(),
    loadReserverQueuesDashboardContext(),
    loadStockOffersDashboardContext(),
  ]);
  // LE FILTRE QUI SÉPARE LES DEUX PRODUITS. Une activité appartient à l'un ou à
  // l'autre, jamais aux deux : `booking_mode` est exclusif. Les salles de
  // « Réservation » n'ont rien à faire dans ce catalogue.
  const activites = agenda.ok
    ? agenda.activities.filter((a) => a.bookingMode === "moment")
    : [];
  ...
  return (
    <div>
      ...
      <div className="mt-8 space-y-8">
        {/* LE PREMIER BLOC EST UNE CARTE TITRÉE, DEPUIS QU'IL PORTE UN RANG.
            ...
            sur la première ligne de la liste, sans rien qui dise de quoi ils
            sont le rang. Elle prend donc la même forme que les trois blocs
            suivants — `<Card>` et `<h2>` — et son état vide le motif de
            `FilesAccueilPanneau` plutôt qu'une carte dans une carte. Le titre
            est celui de la tuile, écrit une seule fois dans
            `TUILES_RESERVER`. */}
            ...
            {activites.length === 0 ? (
              <div className="mt-5 rounded-xl border-2 border-dashed border-k-ink/25 px-4 py-8 text-center">
                ...
                {/* LE BOUTON EST ICI AUSSI : « créez la première » sans rien à
                    cliquer laisse le seul bouton en haut d'écran, hors du regard
              ...
              <ul className="mt-5 space-y-3">
                {activites.map((activite) => {
                  // « Ouverts à venir » au sens du joueur : c'est
                  // `etatUiCreneau` qui en décide, comme sur la page publique.
                  // Un compte calculé autrement ici ferait dire au tableau de
                  // bord « 3 créneaux » là où le client n'en voit qu'un.
                  //
                  // LE FORMAT ENTRE DANS LE VERDICT (RES-5), et sans lui la
                  // phrase ci-dessus cessait d'être vraie : sur un Atelier Duo à
                  // qui il reste UNE place, `etatUiCreneau` rend « complet » —
                  // une place isolée n'est prenable par personne — mais
                  // seulement si on lui DIT que c'est un duo. Sans `kind`, cet
                  // écran comptait le créneau comme ouvert quand la page
                  // publique affichait « complet » dessus : le commerçant lisait
                  // « 1 créneau ouvert à venir » et son client ne pouvait pas
                  // réserver.
                  //
                  // Le format vit sur l'ACTIVITÉ, pas sur le créneau
                  // (`ReserverSlotDashboardView` ne le porte pas, et c'est voulu
                  // — un même Atelier Duo ne peut pas changer d'unité selon
                  // l'heure). On le joint donc ici, à la lecture.
                  const ouverts = activite.slots.filter(
                    (creneau) =>
                      etatUiCreneau({ ...creneau, kind: activite.kind }) ===
                      "ouvert",
                  ).length;
                  return (
                    <li key={activite.id}>
                      ...
                      >
                        <div className="flex items-center justify-between gap-4">
                          ...
                          <div className="flex shrink-0 flex-wrap items-center justify-end gap-2">
                            {/* Le format (RES-5) AVANT l'interrupteur :
                                « Atelier Duo » dit ce que c'est, « Ouverte » dit
                                seulement si ça tourne. Rien ne s'affiche sur une
                                activité standard — c'est le défaut, et une
                                colonne où chaque ligne porte le même mot ne
                                montre plus les deux qui en portent un autre. */}
        ...
        {/* L'écran d'arrivées est SOUS la liste et HORS d'une activité : le code
            d'un client ne dit pas laquelle il a réservée, et demander au
            caissier de choisir d'abord l'activité lui ferait chercher
            l'information qu'il vient précisément d'obtenir. La RPC, elle, résout
            le code dans toute l'organisation — y compris sur une réservation de
            salle, ce qui est voulu : un comptoir ne trie pas ses clients par
            produit. */}
        <CarteRepliable {...carteTuile(tuiles, "arrivees")}>
          <ArriveesCheckin timeZone={timeZone} />
        </CarteRepliable>

        {/* LES FILES D'ACCUEIL SONT SOUS L'AGENDA, ET C'EST L'ORDRE DE LA
            JOURNÉE : on prépare ses créneaux le matin, on tient sa file toute la
            journée. La console est ouverte au CAISSIER — elle ne dépend pas de
            `canEditDraft`, qui ne gouverne que la création et les réglages,
            passés en `peutEditer`.

            Elle vit ICI et nulle part ailleurs : une prise de rendez-vous n'a
            pas de file d'attente, on y a une heure. */}
        <CarteRepliable {...carteTuile(tuiles, "files")}>
          <FilesAccueilPanneau
            files={files}
            // `active` EN PLUS DU NOM : couper une activité referme ses files
            // côté `queue_join`, et la vue d'une file ne porte que son propre
            // `status`. Sans ce drapeau, la pastille lisait « Ouverte » sur une
            // file qui refusait tout le monde.
            ...
            // côté serveur — un identifiant de quiz ou de campagne ne se saisit
            // pas.
        ...
        {/* LES OFFRES DE STOCK EN DERNIER, ET C'EST LA MÊME LOGIQUE DE JOURNÉE :
            on prépare ses créneaux le matin, on tient sa file toute la journée,
            et on solde son invendu en fin de service. Le geste du caissier n'est
            pas ici — le retrait se fait en CAISSE, par le code — d'où l'absence
            de console et le seul droit d'édition. */}
