...
## Last Updated
- **Date**: 2026-08-05
- **Dernier chantier**: **Le QR universel est couvert, et le back-office pouvait ne créer aucun octroi** (branche `chantier/qr-restants`), 2026-08-05. Aucune migration. **Le §4 du cahier est couvert : huit modules équipés, le neuvième JUSTIFIÉ.** La fidélité n'exposait son URL publique nulle part ; l'événement prend le `join_code` et NON l'UUID — celui que la salle lit à voix haute, sans quoi la même soirée aurait deux adresses ; `event-qr.tsx` est lu et non modifié, avec un test qui garde la séparation entre une data-URL projetée PENDANT la soirée et une affiche imprimée AVANT. **Le parrainage n'a PAS de QR, décidé et écrit** : sans `?ref=` c'est le QR de campagne déjà existant, avec un `?ref=` choisi par le commerçant TOUS les scans arrivent parrainés par la même personne, versant les récompenses à un compte arbitraire — deux assertions rougiront si une route `/parrainage/…` apparaît, pour que la décision se rejoue. **Le défaut trouvé en route était plus grave que celui qu'on corrigeait** : `formData.get` rend `null` pour un champ NON RENDU, que `entierOptionnel` rejette — **aucun octroi `recurring` n'était créable depuis le back-office**. **PostgREST ne transmet pas `constraint_name`** : le refus se reconnaît par `code === "23505"` ET l'identifiant de l'index dans le message, jamais la phrase (traduite, délimiteurs variables) ; preuve relevée sur un VRAI 23505 et gravée dans le test. Preuve : typecheck 0, lint 0, casts 0, build vert, **191 fichiers / 3207 tests**. Roadmap V1.38.
  **Reste ouvert** : `entierOptionnel` rejette toujours `null` et le correctif est LOCAL à une action — c'est une classe, non auditée ; les 10 prix Stripe sont en TEST, la chaîne complète n'a pas été éprouvée de bout en bout et le passage en live attend cette preuve.
