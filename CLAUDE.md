...
## Last Updated
- **Date**: 2026-08-23
- **Dernier chantier**: **Sept lots Vitrine et le Ticket d'Or, enchaînés** (PR #180 à #188, migrations `20261023120000` → `20261029120000`). Carte et photos importées **dans le navigateur** — le fichier ne quitte pas l'appareil ; traduction déclenchée par le commerçant, jamais par un visiteur ; mesures **sans identifiant** (compteurs agrégés, ni cookie ni empreinte) ; Boussole à facettes fermées ; sortie après jeu ; indexation Google **sur consentement** ; et le Ticket d'Or, jeu du socle gardé par la clé de l'offre de base. Ce que la vérification a trouvé : un `create or replace` (VIT-7) avait **annulé en production** un patch en place de `20261020120000` — réparé par `20261026120000` ; un capteur d'isolation a refusé deux clés étrangères simples du Ticket d'Or ; les types générés portaient une fonction fantôme d'un brouillon renommé.
  **Reste ouvert** : `CRON_SECRET` absent des secrets GitHub ; 5 packs Bande à relire ; `reward_issuances` sans clé `(id, organization_id)` — dette inscrite dans `fk_composites_couverture`. **Vendable, jamais vendu** : Vitrine (+20 €) et Réserver (+30 €) s'ajoutent en ligne de l'abonnement (`subscriptions.update`) — catalogue LIVE, variables Production et écran en place, doublés en mode TEST le 2026-08-23 ; mais **zéro abonnement LIVE** à ce jour, donc ce parcours n'a jamais tourné en réel. **Geste propriétaire** : poser `GOOGLE_TRANSLATE_API_KEY` (sans elle, VIT-6 ne traduit pas), et révoquer la clé `rk_live_` et le jeton Vercel (`docs/chantier-audit-2026-08-16.md`).
