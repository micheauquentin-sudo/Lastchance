...
## Last Updated
- **Date**: 2026-08-07
- **Dernier chantier**: **Apparence dashboard — clarté et rappels fermables** (branche `chantier/apparence-dashboard`, PR à ouvrir vers `main`, **NE PAS merger** — décision propriétaire —, sans migration), 2026-08-07. Demande propriétaire du jour : améliorer l'apparence/clarté du dashboard, 7 points, livrés en 5 commits. **Shell** (`eaf50a2`) : le débordement horizontal des 8 pages de modules corrigé à la source (slot actions du PageHeader `min-w-0 max-w-full` au lieu de `shrink-0`, 8 formulaires de création bornés `max-w-xl`) ; sidebar `lg:overflow-y-auto` (le bouton Déconnexion redevient atteignable). **Rappels fermables** neufs (`src/lib/rappels.ts` pur+testé, `src/actions/rappels.ts`, `RappelFermable`) : cookie + server action, zéro flash, liste blanche de préfixes de clés — jamais fermables les 3 bandeaux bloquants (incident de paiement, abonnement inactif, essai terminé). **Détail campagne** (`dabf9ec`) : 6 blocs secondaires repliables via `CarteRepliable` (bouton `aria-expanded`, PAS `<details>` — Chromium retire le rôle heading aux descendants d'un `<summary>`) ; QR embarqué sur la page du jeu (vignettes, création pré-remplie, suppression), fin de l'aller-retour vers l'onglet QR Codes. **Titres** (`18dddd1`) : `Card` impose `[&>h2]:text-lg [&>h2]:font-black`, 67 titres alignés sur le style atelier en un point unique. **Accueil dédoublonné** (`4b77353`) : un même compteur s'écrivait jusqu'à 3-4 fois (tuile+tâche+conseil+hero) ; les 4 règles opérationnelles du Conseiller redondantes avec des tuiles supprimées ; Conseiller fermable (clé par condensé des conseils affichés). **Revue sécurité fermée avant PR** (`1cb13a5`) : GO, 0 critique/élevé, 2 MOYEN + 4 INFO — M1 liste blanche de préfixes de cookie, M2 garde de rôle sur les 3 actions QR mutantes, I1 cookie borné à `/dashboard`, I2 purge au logout, I4 clés normalisées ; I3 (ombrage de cookie, XSS requis) et I5 (pas de rate-limit) documentés sans action. Nouveau `src/actions/qr-codes.test.ts` (14 tests). **Preuves** (campagne locale complète, CI pas encore jouée — la PR la jouera) : typecheck 0, lint 0, **237 fichiers / 3806 tests**, migrations/sql/casts check ok, build vert (46 pages) ; E2E ciblé WSL sur dashboard-home/referral/wheel-wizard/campaign-templates : 35 passed / 1 skipped / 1 failed (flake WebKit préexistant sur `cashier : /dashboard redirige vers la caisse`, confirmé par rejeu isolé ×3 vert). Zéro migration.
  **Reste ouvert** : **PR à ouvrir puis fusionner sur décision propriétaire** (comme #125, #126, #127). Préférence de rappel par navigateur et non par utilisateur (borné par la purge au logout) ; l'ancien cookie posé en path `/` chez les premiers utilisateurs survit jusqu'à expiration/logout ; `quiz-editor.tsx:836` et `wheel-style-editor.tsx:199` restés à l'ancien style de titre (réservés au chantier thèmes à venir) ; état de repli des cartes non persisté. Prochain chantier annoncé : fonds thématiques cartoon par thème (décors SVG, thème pronostics avec migration `contests.theme`), en préparation.
