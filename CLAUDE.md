...
## Last Updated
- **Date**: 2026-08-17
- **Dernier chantier**: **Le catalogue Stripe dit vrai — wagon 2 du train de correction de l'audit transverse** (branche `chantier/audit-p0-stripe`, PR à ouvrir, migrations `20260925120000` et `20260926120000`), 2026-08-17. Un pass n'ouvre plus que son module (`org_has_module_access(_for_resource)`, SD-4) ; la Saison de pronostics se vend pour UNE compétition, resserrée à la clôture (SD-5) ; capacité d'un événement lue au `max()` des octrois vivants, paliers 10/30/50 (SD-1) ; un rachat pendant la grâce réactive au lieu de doubler (SD-6) ; webhook Stripe reprend un remboursement/litige et débite le solde SMS, désormais borné par organisation (SD-2) ; garde de checkout par famille de prix — un abonnement 100 % pass ne ferme plus la vente de l'offre (SD-3) ; grâce d'impayé d'un pass ancrée sur l'événement de SON abonnement, jamais `past_due_since`, bornée monotone ; programmation de campagne gardée par le droit du module, motif `droit_expire` visible + e-mail au propriétaire au lieu d'un refus silencieux (SD-9, ADR-103 — décision renversée de la migration `20260906120000`). Preuve : typecheck/lint/build verts, pgTAP 60f/3493 (vide+semée), E2E local WSL mobile-chrome 39/6 skipped, revue sécurité GO après correction des 4 findings de la première passe. ADR-103, roadmap V1.58.
  **Reste ouvert** : 5 INFO dans `docs/bugs.md` (wagon 2). Wagons 3 à 7 à venir — voir `docs/chantier-audit-2026-08-16.md`. Deux gestes propriétaire hérités : révoquer `rk_live_` et le jeton Vercel.
