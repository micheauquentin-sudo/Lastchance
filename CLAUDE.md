...
## Last Updated
- **Date**: 2026-08-17
- **Dernier chantier**: **La boucle joueur → gain se ferme — wagon 3 du train de correction de l'audit transverse** (branche `chantier/audit-p0-joueur`, tête `65c25e5`, PR à ouvrir, migration `20260927120000`), 2026-08-17. Reprise d'un gain non réclamé étendue à toute la fenêtre de `play_limit`, plus de cutoff 30 min (`recover_pending_spin`, JOU-1, calcul recopié de `perform_atomic_spin`, sans miroir TS) ; nonce d'un défi skill-gated devenu clé d'idempotence (`p_idempotency_key`, JOB-8/SEC-2, un défi ne joue qu'une fois) ; 4 écrans « tour offert » gagnent un try/catch réseau (UI-1) et une sortie vers le portefeuille en cas de blocage (UI-2) ; écran caisse jackpot staff écrit (`jackpot-staff-checkin.tsx`, `/dashboard/redeem`) — `participateJackpotStaff` trouve son appelant (MORT-1, arbitrage : écrire plutôt que retirer) ; plancher Jauge à 300 ms + mention honnête sur qui juge le geste (arbitrage : durcir plutôt que retirer). Preuve : pgTAP 61f/3522 PASS ×2, typecheck/lint/build verts, E2E local (spec caisse neuve + non-régression vertes) ; suite Vitest complète et CI GitHub jouées par la CI de PR. Revue sécurité GO (3 MOYEN + INFO-1 fermés avant PR). ADR-104, roadmap V1.59.
  **Reste ouvert** : JOU-7/C4 précisé (consigné, pas un bug), FAIBLE-1 à surveiller (`skill_impossible_timing`), INFO-2 et INFO-4 dans `docs/bugs.md` (wagon 3). Wagons 4 à 7 à venir — voir `docs/chantier-audit-2026-08-16.md`. Deux gestes propriétaire hérités : révoquer `rk_live_` et le jeton Vercel.
