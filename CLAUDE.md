...
## Last Updated
- **Date**: 2026-08-05
- **Dernier chantier**: **Les huit add-ons sont vendables, le QR gagne trois modules, deux silences de type fermés** (branches `chantier/trois-suites` puis `chantier/trois-lots`, migration `20260910120000`), 2026-08-05. **Les deux mensuels sont ouverts** (ADR-081) sur une décision produit — *un commerçant ne peut pas racheter un add-on mensuel déjà actif* —, et cette règle **supprime le problème au lieu de le tracer** : `(organisation, module)` devient la clé de révocation, donc aucun identifiant d'abonnement Stripe n'a besoin d'être persisté ; la colonne et l'index prévus disparaissent du plan. La garde réelle est un **index unique partiel** à prédicat immuable, pas le refus côté action — entre la vérification de l'action et l'écriture du webhook, un double clic ouvre une fenêtre où deux paiements partent. `partitionnerPrix` sépare les prix **avant** toute résolution, conclusion directe d'ADR-079 : faire tolérer le prix inconnu à `resolveStripeEntitlements` l'aurait fait retomber sur `PLANS[0]` et écraser le plan payé. **QR : 2 modules sur 9 → 5** (`PublicShare` généralise `contest-share`, supprimé et non doublé) ; expérience non publiée → aucun QR, parce qu'un QR imprimé survit à la page qui l'a produit. **Clients Supabase serveur typés** : 82 erreurs révélées, 82 fermées en 5 gestes distincts dont 48 **à la racine** en un seul endroit ; 3 zones aveugles closes, dont `runProgressionEditorRpc` où 13 appels passaient sans vérification du nom de RPC. **`database.contract.test.ts` compare enfin la nullabilité**, et la garde est **prouvée sur l'état historique** du dépôt. Preuve : typecheck 0, lint 0, casts 0, build vert, **190 fichiers / 3178 tests**, pgTAP 21 assertions (150 avec voisins). ADR-081 ; ADR-079 marquée levée ; roadmap V1.37.
  **Reste ouvert** : `STRIPE_PRICE_ID_PASS_*` à poser — **geste du propriétaire**, sans quoi aucun bouton n'apparaît ; fidélité, événement et parrainage sans QR commerçant ; un mensuel `past_due` reste ouvert jusqu'à l'annulation Stripe (délibéré) ; le back-office rend un message opaque sur un cumul refusé.
