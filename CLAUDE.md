...
## Last Updated
- **Date**: 2026-08-07
- **Dernier chantier**: **L'Atelier du jeu** (branche `chantier/assistant-creation`, PR #126 ouverte vers `main`, **NE PAS merger** — décision propriétaire —, sans migration), 2026-08-07. Demande propriétaire : accompagnement de création en étapes, guidé et DÉTERMINISTE, sans IA (retrait de l'assistant payant réaffirmé). **Diagnostic préalable** (5 explorateurs) : 102 contrôles interactifs simultanés sur la page roue, 6 actions d'écriture sur 12 boutons Enregistrer sans état global, « Ouvrir aux joueurs » sans précondition métier, 13 mécaniques sur 15 sans effet visible des réglages, aucune spec E2E ni scan axe sur cette page. **Livré** : `/dashboard/campaigns/[id]/wheel` devient l'Atelier — 5 étapes nommées (Le jeu / Les lots / L'habillage / Le créneau / La vérification) navigables par `?etape=` sur la MÊME route (les 6 revalidatePath restent valides), zéro nouvelle action serveur, zéro migration — chaque étape poste une sauvegarde EXISTANTE complète (le trio game_type+play_limit+skill_config reste insécable). Étape Jeu : radiogroup 2 familles honnêtes (hasard / défi joueur, échec=perdant), « Illimité » désactivé sur jeux à secret. Étape Vérification : checklist pure testée (20 tests), lot gagnant tirable au miroir de perform_atomic_spin, le CTA mène au seul endroit qui publie (#statut) — la publication reste hors de l'Atelier. Catalogue de mécaniques et calcul partSur10 extraits en modules purs testés, résorbant 3 copies divergentes. Couture : `createCampaign` atterrit dans l'Atelier ; `applyCampaignTemplate` garde le détail. **Nouvelle spec** `e2e/wheel-wizard.spec.ts` (8 tests, premier scan axe de cette page) a débusqué 13 violations RÉELLES préexistantes (contrastes, selects/case/curseur sans nom accessible), corrigées à la racine. **CI complète VERTE sur `0faa05a`** (run 31167771881 : E2E 3 navigateurs, pgTAP/RLS, CodeQL, typecheck/lint/Vitest/build, audit). Revue sécurité dédiée non requise (aucune migration/route API/auth/RLS/webhook/token touchés — seule la cible d'un redirect interne change). Preuve : typecheck 0, lint 0, **225 fichiers / 3654 tests**, build vert. ADR-090, roadmap V1.46.
  **Reste ouvert** : **PR #126 à fusionner sur décision propriétaire** (comme #125). Préconditions de publication en base à arbitrer (set_campaign_status sans garde métier). Aucun paiement réel mené de bout en bout ; deux gestes propriétaire restent : révoquer la clé `rk_live_` et le jeton de contournement Vercel.
