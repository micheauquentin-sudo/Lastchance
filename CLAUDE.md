...
## Last Updated
- **Date**: 2026-08-17
- **Dernier chantier**: **Le commerçant garde la main, les chiffres disent vrai — wagon 4 du train de correction de l'audit transverse** (branche `chantier/audit-p1-controle`, tête `86db376`, PR ouverte, migration `20260928120000`), 2026-08-17. `start_event_session` refuse un lobby non `active` (FIA-1) ; `updateCampaign`/`updateContest` refusent une publication sans lot gagnant tirable ni poids, ou sans match — gardes **applicatives**, l'ADR-105 ferme l'arbitrage ouvert par l'ADR-090 (FIA-2) ; `set_campaign_status` désarme `auto_schedule` et refuse `archived → active` sur la seule campagne, six écrans restent permissifs par choix, prouvé par pgTAP (FIA-3, FIA-6) ; suppression du dernier lot gagnant refusée (FIA-5) ; suppression d'une question de soirée confirmée, impossible en direct (EXP-3) ; clés d'analytique datées, rupture de série assumée (NUM-1) ; tuile « Scans QR » nommée juste (SCAN-1) ; hero/tâche masquée alignés (EXP-2) ; listes de module bornées (LIST-1) ; 3 index de FK (IDX-1) ; plafond de page 500, TS et RPC (CNT-1). Preuve : typecheck/lint/build verts, Vitest complet vert, pgTAP 62f/3561 PASS ×2, E2E mobile-chrome 41/3/0. Sécurité GO (1 MOYEN + 2 FAIBLE + 7 INFO, MOYEN et 1 FAIBLE fermés). ADR-105, roadmap V1.60.
  **Reste ouvert** : 7 points sans correctif dans `docs/bugs.md` (wagon 4) + F-2/INFO-3/INFO-5 sécurité. Wagons 5 à 7 à venir — voir `docs/chantier-audit-2026-08-16.md`. Deux gestes propriétaire hérités : révoquer `rk_live_` et le jeton Vercel.
