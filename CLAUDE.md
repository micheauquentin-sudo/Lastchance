...
## Last Updated
- **Date**: 2026-08-29
- **Dernier chantier**: **Réservation de table : un plan de salle, pas une jauge de couverts** (RDV-6 à RDV-9, PR #229 → #231 fusionnées, #232 en CI). Réservation (`rendez_vous`) a désormais des tables nommées et un effectif exact : `reserve_table` fait un meilleur ajustement, `reservation_tables_state` rend le plus grand effectif **plaçable** (jamais une somme de couverts libres), et `table_turn_minutes` reste distinct de `duration_minutes`. La liste d'attente **notifie**, elle ne tient pas de table. RDV-6 a aussi réparé une omission de RDV-5 : `rendez_vous` manquait dans `organization_module_grants_module_check`.
  **Ce que la vérification a trouvé** : deux colonnes RDV-6 (`reservations.table_id`, `reservation_waitlist_entries.party_size`) avaient été oubliées du grant de lecture colonne par colonne — PostgREST refuse **en entier** tout `select` qui les touche sans lui, et la panne se serait lue « les réservations ont disparu ». Trouvées par pgTAP après coup, corrigées avec une garde jumelle chacune ; aucune garde générique ne couvre la prochaine omission (`docs/bugs.md`).
  **Reste ouvert** : le socle Moments (`reserve_slot`, `waitlist_join`, `reservation_offer_next`) vérifie encore `vitrine`, pas `rendez_vous`. **Geste propriétaire** : créer le produit Stripe « Réservation » (20 €/mois) et poser `STRIPE_PRICE_ID_ADDON_RENDEZ_VOUS` en Production.
