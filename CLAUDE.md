...
## Last Updated
- **Date**: 2026-08-22
- **Dernier chantier**: **Sur Place, la cinquième offre — quatre produits livrés cessent d'être invendables** (ADR-117, migration `20261021120000`). `vitrine`, `reserver`, `duo` et `bande` étaient en production et dans aucune offre ; `place` / « Sur Place » (79 €/mois = 29 + 20 + 30, sans remise) les porte, et La Totale les absorbe **sans changer de prix**. **Les tarifs de jeu ne bougent pas** : le relevé concurrentiel les a validés — le jeu au QR se vend 9,90 €/mois, la fidélité plafonne à 49 €. Ce que la vérification a trouvé : l'audit disait « aucune migration nécessaire », or `apply_stripe_subscription_event_v2` aurait levé « invalid plan » puis « invalid entitlement » dès le PREMIER abonnement vendu — et un webhook en échec est rejoué trois jours avant que Stripe coupe le point d'entrée, emportant la synchro des abonnements **existants**. Deux registres neufs : `MODULE_CATALOG` et `duo`/`bande` promus au rang de droits.
  **Reste ouvert** : les options Vitrine (+20 €) et Réserver (+30 €) ne sont pas vendables — il manque un `subscriptions.update` (une option mensuelle crée aujourd'hui un abonnement Stripe **séparé**) et un prix Vitrine ouvrant trois colonnes. Inchangés : `CRON_SECRET` absent des secrets GitHub, aucune présence dans les salons, `robots: index false` sur la Vitrine, 5 packs Bande à relire. **Gestes propriétaire** : créer le produit/prix Stripe « Sur Place » en LIVE puis poser `STRIPE_PRICE_ID_PLACE` sur Vercel (la clé `rk_live_` de la CLI n'a pas `product_write`), et révoquer cette clé et le jeton Vercel (`docs/chantier-audit-2026-08-16.md`).
