...
## Last Updated
- **Date**: 2026-08-08
- **Dernier chantier**: **Retours propriétaire — six demandes sur V1.48/V1.49** (branche `chantier/retours-proprietaire`, PR à ouvrir vers `main` — fusion sur l'ordre permanent du propriétaire « migrer tout dès la réponse de la CI » —, migration `20260918120000_invitation_prejeu.sql`), 2026-08-08. Six retours livrés en 12 commits. **Fonds redessinés** : le premier jet V1.49 (motifs 12 % d'alpha) rejeté sur capture ; refonte en lavis PAR thème (`theme-lavis.ts`), personnages à visages, motifs à 0,80-0,95 d'opacité, contraste mesuré au pire pixel du motif (plancher 7:1, pire cas réel 10,1:1). **L'aperçu montre le jeu choisi** : `GameIdleScreen` extrait du socle et rendu à la fois par les 4 coquilles joueur et l'aperçu éditeur ; personnalisation optionnelle `wheels.style.games` (zéro migration, absence = rendu historique inchangé) pour 6 mécaniques. **Calendrier : case vide = « pas de chance »**, plus un blocage — l'invariant V1.47 retiré, publication libre, deux contrôles non bloquants conservés (cases-vides nommées+liées, garde-fou d'assiduité) ; l'ouverture d'une case vide compte dans `opened_count`. **Invitation avant-jeu** : proposer de noter Google / suivre Instagram-TikTok avant un jeu instantané, structurellement non bloquante (bouton « Continuer » jamais désactivé, verrouillé par test) ; liens à l'établissement (migration, CHECK https+300+liste blanche d'hôtes bornée au chemin — writereview/maps/maps.app.goo.gl/g.page, `g.co` et ports refusés), activation par campagne ; porte bloquante précédente et FAQ obsolète nettoyées. **Navigation d'étapes** en haut ET en bas des 8 ateliers. **Titres** : trait de marqueur jaune sous chaque titre de carte (exception « Zone dangereuse » en rouge). **Revue sécurité : GO, 0 critique/élevé, 1 MOYEN fermé avant PR** (liste blanche Google resserrée au chemin exact), 2 INFO fermés, 2 INFO documentés (parse-avant-garde du fichier, dette `TRUNCATE` table-level héritée de la migration `00018`, hors périmètre). Preuve : typecheck 0, lint 0, **247 fichiers / 3926 tests**, build racine+site vert, migrations:check 122/tête `20260918120000`, pgTAP **56 fichiers / 3196 assertions** PASS vide+semée (dont les 15 ACL invitation et les 9 calendrier, jouées pour la première fois), E2E desktop-smoke WSL 42/42 sur le commit final. ADR-094/095, roadmap V1.50.
  **Reste ouvert** : mode TV pronostics sans thème ; branche nuit de `/play` sans décor (le lavis par preset reste à arbitrer) ; préférence d'invitation par navigateur (`sessionStorage`) ; valeurs de liens déjà en base hors nouvelle liste blanche cessent d'être servies (impact nul, prod = 1 org de test) ; dette `TRUNCATE` table-level à traiter par `db-supabase`. Deux gestes propriétaire restent : révoquer la clé `rk_live_` et le jeton de contournement Vercel.
