...
## Last Updated
- **Date**: 2026-08-06
- **Dernier chantier**: **L'assistant de création, dormant sans clé** (branche `chantier/ia-assistant-creation`, sans migration), 2026-08-06. Point 5 — DERNIER — de l'ordre impératif du cahier (§9.5) : **l'ordre impératif est désormais complet.** Assistant de création uniquement (§6) : aide au choix + trois idées de campagne ÉDITABLES, sortie structurée côté serveur, sans PII joueur, sans publication/paiement automatique — l'IA propose, le commerçant valide. Aucune migration. **Appel REST DIRECT à l'API Messages d'Anthropic, SANS SDK** — aucune dépendance ajoutée. Modèle **claude-haiku-4-5**. `src/lib/ia-provider.ts` : `getIaProvider()`/`isIaConfigured()` calqués sur `getSmsProvider()`/`isSmsConfigured()` — clé `ANTHROPIC_API_KEY` en environnement, jamais journalisée ni renvoyée à l'écran ; `import "server-only"`. `stop_reason` vérifié avant lecture de `content` ; non-200/refus → liste vide, timeout dur 20 s. **DORMANT SANS CLÉ** : refuse « Assistant indisponible » AVANT tout fetch, l'UI n'affiche AUCUN bouton — la feature s'allume à la pose de la clé, comme la vente d'add-ons s'allume à la pose des prix Stripe. **Sortie validée zod** : `campaignBlueprintSchema` (déjà revalidé aux deux bouts) sert de contrat ; JSON Schema dérivé de zod impraticable (superRefine + bornes non supportés) → prompt + parse + `safeParse` idée par idée, idée invalide ÉCARTÉE. **PII blanc-listée à la main** : littéral composé champ par champ (nom, type saisi, objectif enum), JAMAIS l'objet Organization entier, jamais de donnée joueur — vérifié avec secrets-appâts. **Rate limit** : `iaSuggestionRequest` (org+user, 10/h, failClosed, ADR-032) et `iaSuggestionOrg` (org, 30/h). **Application** : `applyCampaignTemplate` gagne une source `blueprint` (exclusive de templateKey/templateId), revalidée par campaignBlueprintSchema — brouillon inerte, rien de publié. Greffe UI sur la galerie (`src/app/dashboard/campaigns/page.tsx`). **Revue sécurité : GO, 0 critique/élevé/moyen, 3 INFO non bloquants** (bugs.md) — INFO-1 `applyCampaignTemplate` sans seau par opérateur (pré-existant) ; INFO-2 texte du modèle échappé par React ; INFO-3 pas de garde de rôle UI (côté serveur oui). Aucun corrigé. Preuve : typecheck 0, lint 0, casts:check 0, migrations:check 120/tête `20260916120000` (aucun SQL), sql:check ok, **221 fichiers / 3579 tests**, build vert. ADR-088, roadmap V1.44.
  **Reste ouvert** : feature dormante — aucun appel réel à Anthropic éprouvé de bout en bout (pas de clé en CI/prod). Aucun paiement réel mené de bout en bout (déjà consigné). Deux gestes propriétaire persistants : révoquer la clé `rk_live_` et le jeton de contournement Vercel.
