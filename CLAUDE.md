...
## Last Updated
- **Date**: 2026-08-05
- **Dernier chantier**: **P0.4 — un paiement crée un octroi, six add-ons deviennent achetables seuls, et un pass payé peut démarrer** (branche `chantier/p0-4-achat-octrois`, 9 commits, migrations `20260908120000` et `20260909120000`), 2026-08-05. Ferme la limite que P0.2 laissait ouverte : « aucun flux de paiement/webhook ne crée encore ces octrois ». Côté réception, `grant_module_from_payment` insère par `on conflict do nothing` et **ne met pas à jour** — Stripe rejoue ses webhooks, et un `do update` aurait redaté la fenêtre, ouvrant soixante jours pour trente payés. Les termes viennent du **catalogue**, jamais de la metadata. Côté émission : `octroi-checkout.ts` (variables `STRIPE_PRICE_ID_PASS_*`, distinctes de celles de l'abonnement), `createAddonCheckoutSession`, `/dashboard/settings/modules`. **Le geste qui manquait, trouvé sur une question du propriétaire** (« les durées ne sont pas toutes les mêmes ») : rien ne faisait sortir un octroi de l'état `pending`, donc **cinq add-ons sur six encaissaient sans rien ouvrir**, et `activeDays` n'était lu que par l'affichage du tarif — invisible à typecheck, lint, 3121 tests, build et pgTAP, parce que chaque pièce était correcte séparément et que c'est le geste qui les relie qui manquait. `termesActivation` + `activate_module_grant` appliquent enfin les deux durées du §2 (30/31/7/30 jours, et 8 pour la Soirée en jeu). Preuve : typecheck 0, lint 0, build vert, **187 fichiers / 3126 tests**, pgTAP 19 + 14 assertions sur base réelle. ADR-079, ADR-080, roadmap V1.36.
  **Reste ouvert** : les **deux add-ons mensuels ne sont pas vendables**, et la garde qui les ferme (`venteEnLigneOuverte`) est un contournement, pas un correctif. Les vendre créerait un abonnement Stripe séparé dont le prix est inconnu de `resolveStripeEntitlements` → 500 en boucle, ce qui couperait aussi la synchro des abonnements principaux ; **l'ignorer serait pire** — retombée sur `PLANS[0]`, donc écrasement du plan payé. S'y ajoute qu'aucune révocation d'octroi `recurring` n'existe à la résiliation. Trois gestes pour lever la garde, décrits dans `docs/bugs.md` et ADR-079. **Et aucun prix Stripe n'est posé** : sans variable, la page affiche huit options et zéro bouton — livrable à froid, la vente s'allume au geste du propriétaire.
