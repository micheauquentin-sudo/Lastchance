...
- **Dernier chantier**: **Sept lots Vitrine, le Ticket d'Or, puis la panne qu'ils avaient posée** (PR #180 à #191). Les huit lots : carte et photos importées **dans le navigateur**, traduction déclenchée par le commerçant, mesures **sans identifiant**, Boussole à facettes fermées, sortie après jeu, indexation Google **sur consentement**, Ticket d'Or (jeu du socle). Puis #191, trouvée dans les journaux Vercel : `sharp` ne se chargeait pas en production (`libvips` non embarqué), et un module natif qui échoue emporte **tout le fichier qui l'importe** — enregistrer l'ADRESSE d'une vitrine, sans la moindre image, échouait donc aussi. Chargement différé plus `outputFileTracingIncludes`, vérifiés sur une construction Linux.
  **Ce que la CI ne disait pas** : un push **direct sur `main`** ne déclenche que « Santé après déploiement ». Quatre gardes y sont passées au rouge sans que personne le voie — elles n'échouaient que sur les PR d'autrui. Dont le webhook Stripe, qui rendait 500 : onze arguments envoyés pour douze paramètres, `cancel_at_period_end` absent devenant « fonction introuvable ». Toutes réparées.
  **Reste ouvert** : `CRON_SECRET` absent des secrets GitHub ; 5 packs Bande à relire ; `reward_issuances` sans clé `(id, organization_id)`, dette inscrite dans `fk_composites_couverture` ; **zéro abonnement LIVE** — Vitrine (+20 €) et Réserver (+30 €) sont configurées de bout en bout (catalogue LIVE, variables Production, mode TEST posé le 2026-08-23), mais ce parcours n'a jamais tourné en réel. **Gestes propriétaire** : poser `GOOGLE_TRANSLATE_API_KEY`, et révoquer la clé `rk_live_` et le jeton Vercel (`docs/chantier-audit-2026-08-16.md`).
