...
## Last Updated
- **Date**: 2026-08-18
- **Dernier chantier**: **Léger, accessible, des états partout — wagon 6 du train de correction de l'audit transverse** (branche `chantier/audit-p2-front`, tête `ad7600f`, PR à ouvrir, AUCUNE migration), 2026-08-18. Poids client mesuré avant/après (`scripts/mesurer-bundle.mjs`) : quiz éditeur 318,6→117,8 Ko gzip, progression 232,0→31,0, quiz joueur 182,8→43,1, `/play` 187,9→114,0 — PostHog sous consentement accordé, imports `node:crypto` cassés (règle ESLint + garde), résolutions client de roue supprimées, logo 256 px au prochain upload. `error.tsx` sur toutes les routes joueur/public — 2 frontières préexistantes (dashboard, admin) avalaient leurs erreurs en silence, réparées en bonus ; `loading.tsx` sur les 4 routes sans `notFound()` ; **aller-retour à 3 tentatives** (ADR-107) : Next 16 streame les métadonnées, un `loading.tsx` de groupe rendait 200 sur une ressource inconnue — retenu : aucune frontière `loading` au-dessus d'une route publique à `notFound()`, garde retournée ; dashboard/admin assumés. Contrastes recalibrés, scans axe dans 7 specs + `a11y.spec.ts` (exclusion nommée, 17 sites justifiés) — 2 vrais défauts corrigés. Preuve : verif-complete 13/13, E2E vert, pgTAP inchangé. Sécu GO. ADR-107, roadmap V1.62.
  **Reste ouvert** : 4 points dans `docs/bugs.md` (wagon 6 : logos legacy, 404 dashboard/admin assumé, portée ESLint, artefacts a11y en CI). Wagon 7 à venir, voir `docs/chantier-audit-2026-08-16.md`. Gestes hérités : révoquer `rk_live_` et le jeton Vercel.
