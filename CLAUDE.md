...
## Last Updated
- **Date**: 2026-08-17
- **Dernier chantier**: **La soirée live tient sa promesse — wagon 5 du train de correction de l'audit transverse** (branche `chantier/audit-p1-live`, tête `f48b83d`, PR à ouvrir, migration `20260929120000`), 2026-08-17. RPC `event_etat_partage` (part commune, cachée 1 s par session et instance, garde fusionnée) et `event_etat_joueur` (bloc `you`, jamais caché, jamais appelée seule) remplacent trois allers-retours par un seul, équivalence prouvée par pgTAP avec/sans jeton (EVT-1, EVT-2) ; jauge vendue `full`/`live` redescendue de 1000 à **500**, accès offert resté à 1000 — union `100 | 500` de `src/lib/plans.ts` (VEN-1) ; pression jackpot observée après verdict, hors chemin de réponse (JOU-4, JOU-5) ; bandeau « Reconnexion… » + bouton téléphone joueur, sans bouton écran salle, assumé ; chrono ancré serveur ; jauge jackpot par action ciblée, plus de `router.refresh()` ; `docs/perf-report.md` §7 cesse de décrire un cache et un A/B qui n'avaient jamais existé (DOC-1) ; `EVENTS_REALTIME_ENABLED` documenté dans `.env.example`, activation prod = geste d'exploitation. Preuve : pgTAP 63f/3614 PASS ×2, `verif-complete.sh --rapide` 0 échec, E2E ciblé 57/57 verts/3 navigateurs. Sécurité GO (2 MOYEN + 1 FAIBLE + 5 INFO, fermés en partie). ADR-106, roadmap V1.61.
  **Reste ouvert** : 4 INFO dans `docs/bugs.md` (wagon 5) + exclusion ESLint mineure. Wagons 6-7 à venir, voir `docs/chantier-audit-2026-08-16.md`. Gestes propriétaire hérités : révoquer `rk_live_` et le jeton Vercel.
