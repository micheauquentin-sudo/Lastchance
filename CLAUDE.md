...
- **Dernier chantier**: **La Vitrine devient un atelier, et les deux salons deviennent vendables seuls** (VIT-14 à VIT-18, DUO-1 à DUO-3b — PR #281→#290, migrations `20261123120000` à `20261127120000`, ADR-128 à 135). L'écran Vitrine empilait neuf cartes repliables : il devient un **atelier par étape**, avec le **QR devant le statut** — seul module où il passe premier. Elle se **supprime** aussi (RPC, propriétaire seul).
  ...
  **Reste ouvert** : la CSP ci-dessus, et `/vitrine-studio` hors de `SENSITIVE_PREFIXES` ; `duo_choose` ne valide pas une place saisie ; **Stripe — trois produits à créer** (« Réservation » 20 €, Duo et Bande 12 €/mois) et l'émetteur Google Wallet.
