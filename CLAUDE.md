...
## Last Updated
- **Date**: 2026-08-06
- **Dernier chantier**: **Les dernières dettes : une valeur plutôt qu'une nullabilité, la chasse par étape, et le vocabulaire aligné** (branches `chantier/dernieres-dettes` puis `chantier/outcome-et-vocabulaire`, migrations `20260912120000` et `20260913120000`), 2026-08-06. **`grant_module_from_payment` distinguait ses trois issues par la NULLABILITÉ de `grant_id`** — or Postgres ne la transporte pas dans un `returns table` : le générateur écrivait `grant_id: string` non-nullable, le webhook compensait par un cast d'apparence redondante, et une garde textuelle empêchait qu'on le supprime. **On protégeait un correctif au lieu d'ôter la cause.** `outcome text` ('created' | 'replayed' | 'refused') porte désormais la distinction ; `created` disparaît, il était exactement `outcome = 'created'`. **LE `DROP` + `CREATE` EMPORTE LES PRIVILÈGES** (ADR-082), et c'est la trouvaille la plus coûteuse du lot : après recréation `has_function_privilege('public', …)` repasse à `true`, si bien que sans réémission des `REVOKE` une fonction `security definer` **qui octroie des modules payants** redevenait appelable par `anon`. Le pgTAP le vérifie désormais en interrogeant le catalogue après application — l'oubli est **invisible au diff**. Un trou que l'ancien encodage ne pouvait pas voir est fermé au passage : `created` étant booléen, **tout ce qui n'était pas `true`, ligne absente comprise, retombait sur « rejeu »** — le double paiement se serait tu. **La chasse au trésor compte par étape** (ADR-083), et le grain était **déjà tranché** : pour `events`, `resource_id` portait déjà un sous-objet, donc ni colonne ni table ajoutée — avant d'élargir un schéma pour un cas qu'on croit particulier, relire ce qu'il fait des cas voisins. **`/api/scan` → `/api/page-opens`**, renommage sec sans alias (le compteur ne facture rien ; un alias qu'on oublie devient permanent) ; deux détails auraient cassé en silence — `src/proxy.ts` excluait `api/scan` du middleware, et côté `sendBeacon` **un 404 est indiscernable d'un 204**, donc un test vérifie que le chemin appelé existe. Preuve : typecheck 0, lint 0, casts 0, build vert, **194 fichiers / 3258 tests**, pgTAP **52 fichiers / 3020 assertions**, `security:audit-db` 535. ADR-082, ADR-083, roadmap V1.39 et V1.40.
  **Reste ouvert** : **aucun paiement réel n'a été mené de bout en bout** — les 14 prix live sont posés, le webhook écoute ses six événements, mais la chaîne complète (carte → webhook → octroi → Démarrer → module ouvert) n'a jamais tourné d'un trait ; c'est le seul maillon que personne n'a vu fonctionner. Et deux gestes propriétaire : révoquer la clé `rk_live_` (droit d'écriture sur produits, prix et webhooks) et le jeton de contournement Vercel.
