...
## Last Updated
- **Date**: 2026-08-07
- **Dernier chantier**: **Fonds thématiques cartoon** (branche `chantier/themes-cartoon`, PR #129 **fusionnée** vers `main` — ordre propriétaire « migre tout dès la réponse de la CI », CI complète verte —, migration `20260917120000_themes_saisonniers.sql`), 2026-08-07. Demande propriétaire : quand un thème est choisi (Noël, Saint-Valentin…), le fond doit suivre — décors cartoon au lieu de lignes fades, sur toutes les surfaces et aussi pour les pronostics. **Livré** (4 commits) : **DB** `contests.theme` (défaut neutre, CHECK 6 clés saisonnières — même palette que `calendars.theme`, jamais deux vocabulaires pour une idée), liste blanche UPDATE des pronostics réémise avec `theme`, CHECK calendrier élargi à `saint_valentin`, 29 assertions pgTAP dédiées. **Backend** `updateContest` accepte `theme` en optionnel-préservant (absent du FormData ⇒ colonne intacte, la classe du bug `default_locks_at` ne peut pas se reproduire) ; `src/lib/seasonal-theme.ts` = source unique de l'enum (repli neutre en lecture, refus en saisie), `lib/calendar.ts` la consomme au lieu de sa copie. **Frontend** `ThemeDecor` (16 scènes, 28 motifs cartoon-burst, 13 emplacements déterministes sans `Math.random` ni id SVG), `PlayerPageShell` factorise les 4 shells joueur, `/play` et les 3 aperçus éditeurs (calendrier, quiz, roue) rendent le même décor que le joueur, pronostics gagne un sélecteur 6 vignettes et une Saint-Valentin restylée (trame de cœurs), `/pronos/[slug]/recover` gagne son `<main>` manquant. **Durcissement** : `Object.hasOwn` sur les 3 tables de tokens contre une clé héritée du prototype (INFO-1 de la revue, préexistant sur calendrier/quiz, fermé partout). **Revue sécurité dédiée : GO, 0 critique/élevé/moyen/faible, 4 INFO** (1 corrigée avant fusion, 3 en suivi `docs/bugs.md` : ordre de déploiement migration→build, parité palette SQL↔TS non testée entre les deux côtés, garantie optionnel-préservant qui porte sur l'absence et non le vide). Preuve : typecheck 0, lint 0, **238 fichiers / 3803 tests**, build vert, migrations:check 121/tête `20260917120000`, sql:check ok, casts:check ok, pgTAP **56 fichiers / 3172 assertions** PASS vide+semée, E2E ciblé (3 projets, pronostics/calendar/quiz/player-win) 42 passed / 6 skipped / 0 failed. ADR-093, roadmap V1.49.
  **Reste ouvert** : le conflit de docs annoncé avec la PR #128 a été **résolu à la fusion** — #128 (V1.48/ADR-092) fusionnée d'abord (`0c018fd`), `main` mergé dans cette branche, ce chantier renuméroté **V1.49/ADR-093**. Migration à appliquer avant la promotion du build (INFO-2, consigné bugs.md). Quiz sans thème saisonnier (décision produit — reste sur ses 7 thèmes d'usage), branche nuit de `/play` sans décor, mode TV pronostics neutre. Deux gestes propriétaire restent : révoquer la clé `rk_live_` et le jeton de contournement Vercel.
