...
## Last Updated
- **Date**: 2026-08-07
- **Dernier chantier**: **L'Atelier partout — extension aux 7 modules de création** (branche `chantier/atelier-modules`, PR #127 ouverte vers `main`, **NE PAS merger** — décision propriétaire —, sans migration), 2026-08-07. Ordre propriétaire après fusion de V1.46 : étendre le modèle atelier aux autres modules. **Livré** : les 7 modules (quiz, calendrier, chasse, fidélité, jackpot, événement live, pronostics) passent au patron des DEUX VISAGES sur leur route détail : URL nue = vue suivi (Carte de l'Aventure, statut, QR/stats/classement, relance, porte « Ouvrir l'atelier ») ; `?etape=` = atelier une carte à la fois, stepper Kermesse. Primitives V1.46 généralisées, la roue migrée sans changer de comportement (`e2e/wheel-wizard.spec.ts` vert sans modification). Zéro migration, zéro nouvelle action : chaque étape poste une action existante complète ; les 5 cartes Réglages monolithiques restent des étapes indivisibles. Découpages : quiz 4 (tirage sorti vers le suivi), calendrier 3 (vérification NOMME et LIE la case fautive), chasse 4, fidélité 4, jackpot 3 (stepper adaptatif ; écran comptoir conditionné au mode), événement 4, pronostics 6 (vue nue préservant classement/pagination/clôture/palmarès). **Une seule vérité de publication** : préconditions extraites en modules purs testés `src/lib/activation/` (7 modules), consommées par l'action ET l'étape vérification ; pronostics n'avait rien côté serveur. **Bugs corrigés** : pronostics effaçait `default_locks_at` sur no-op (hidden pré-rempli, bouton grisé prouvé par E2E) ; cinq 404 injustifiés sur le droit payé (capacitesDuModule + ModuleCapabilityNotice) ; deux ancres #reglages menteuses ; écran comptoir jackpot hors de son mode. **Filet neuf** `e2e/atelier-modules.spec.ts` (19 tests, premiers scans axe de ces pages) a fait fermer sur trois tours de CI des violations de contraste préexistantes (dette orange V1.45 pelée sur ces surfaces) et débusqué un invariant : une case de calendrier ne peut pas devenir invalide par édition. **CI complète VERTE sur `93319ea`** (run 31188136154). **Revue sécurité dédiée : GO, 0 critique/élevé/moyen**. Preuve : typecheck 0, lint 0, **235 fichiers / 3775 tests**, build vert. ADR-091, roadmap V1.47.
  **Reste ouvert** : **PR #127 à fusionner sur décision propriétaire** (comme #125 et #126). Schémas monolithiques non assouplis, garde de publication en base absente pour pronostics, 3 formulaires updateContest non fusionnés, leaderboard quiz non lu, createLoyaltyOrderCodes sans garde propre (impact nul). Deux gestes propriétaire restent : révoquer la clé `rk_live_` et le jeton de contournement Vercel.
