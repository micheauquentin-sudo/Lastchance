...
| Exécuter tous les E2E sur Chromium et WebKit | 🟡 | Verts sur les deux moteurs. **Le bloc `describe.serial` décrit ici N'EXISTE PLUS** : `e2e/progression.spec.ts` a été réécrit en trois blocs indépendants sur saison semée, et le défaut de production qu'il révélait (clôture de saison invisible) est corrigé (PR #37). Reliquat : un flaky résiduel **atténué, non éteint** — voir docs/bugs.md. Docker et la CLI Supabase tournent désormais en local (WSL2), la CI n'est plus le seul juge. |
...
| Tests | 🟡 | Suite pgTAP et tests unitaires verts, exécutés en local contre un vrai Postgres **et** en CI. Les chiffres ne sont plus recopiés ici : ils changent à chaque chantier et cette ligne a été fausse plusieurs fois. `e2e/progression.spec.ts` a été réécrit (plus de `describe.serial`) ; reliquat de flaky consigné dans docs/bugs.md. |
